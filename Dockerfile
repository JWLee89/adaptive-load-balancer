...
COPY . .
# Set Env variables
RUN cargo build --release --bin adaptive-load-balancer

# We do not need the Rust toolchain to run the binary!
# Start with a minimal image and copy over the binary and assets folder.
FROM debian:buster-slim AS runtime
WORKDIR /app
COPY --from=builder /app/target/release/adaptive-load-balancer /usr/local/bin
# ENV REDIS_HOST_NAME=redis
ENTRYPOINT ["/usr/local/bin/adaptive-load-balancer"]
